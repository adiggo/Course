...
/* Function to do the calculation with a single loop */
void parallelSL ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list )
{
  /* Variables for timing */
  struct timeval ta, tb, tresult;
  ...
  printf ("Parallel Single Loop took %lu seconds and %lu microseconds.  Filter length = %d\n", tresult.tv_sec, tresult.tv_usec, filter_len );
...
//  input_array = (unsigned int*) posix_memalign ( DATA_LEN * sizeof(unsigned int), 4096);
  ...
  /* Execute with single loop */
    int filter_len = 128;
    serialDataFirst ( DATA_LEN, input_array, serial_array, filter_len, filter_list );
    memset ( output_array, 0, DATA_LEN );

    omp_set_num_threads(4);
    parallelSL( DATA_LEN, input_array, output_array, filter_len, filter_list );
